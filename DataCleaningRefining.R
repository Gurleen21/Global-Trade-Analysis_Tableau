...
#loading the dataset
trade1 <- read.csv("Raw Data_1.csv")
trade2<-read.csv("Raw Data_2.csv")

#View dataset
View(trade1) 
View(trade2) #there are zeros in Weight and Quantity columns
...
names(business1) <- c("GlobalTradeID", "Year", "Commoditycode", "Commodity", "Flow")
names(business2)<-c("GlobalTradeID", "Country" , "Flow" , "Dollars" , "Weight", "Quantityname", "Quantity", "Category")      
...
#summary of data
summary(business1)
is.na.data.frame(business1)
sum(is.na.data.frame(business1))

summary(business2)#Missing values present in Weight and Quantity
#There is a "No Quantity" value in Quantityname
#No Quantity indicates Quantity is either 0 or NA
is.na.data.frame(business2)
sum(is.na.data.frame(business2))

#Approach 1: Eliminate all missing values
trader1 <-  na.omit(trade1)
trader2 <- na.omit(trade2)#if missing values are eliminated, we have 58058 obs.

#Joining two Datasets
inner_join(trader1,trader2, .by.='GlobalTradeID')
...
business2$zeroWQ <- ifelse(business2$Weight < 1 & business2$Quantity < 1, "1", "0")
table(business2$zeroWQ)# there are 847 rows that account to this scenario
business2$zeroWQ <-NULL

#When Weight is zero and Quantity is has NA, output is 1 else 0
#we can discard this condition
business2$zeroW_naQ <- ifelse(business2$Weight < 1 & business2$Quantity == NA, "1", "0")

table(business2$zeroW_naQ)#56590 observations says "0"
...
business2$zeroW_naQ <- NULL

#combine multiple scenarios:


#When Weight is zero, or NAs in Weight, or Quantity is zero, or NAs in Quantity - assign 1
business2$Missing <- ifelse(business2$Weight < 1 | is.na(business2$Weight) 
                           | business2$Quantity < 1 | is.na(business2$Quantity), "1", "0")
table(business2$Missing)
...
missedinfo_aus <- filter(business2, Country == "Australia", Missing == 1)
...
missedinfo_can <- filter(business2, Country == "Canada", Missing == 1)
...
missedinfo_usa <- filter(business2, Country == "USA", Missing == 1)
...
#AUSTRALIA
totalusd_aus <- filter(business2, Country == "Australia", Dollars >= 1)
...
#CANADA
totalusd_can <- filter(business2, Country == "Canada", Dollars >= 1)
...
#USA
totalusd_usa <- filter(business2, Country == "USA", Dollars >= 1)
...
#tableau$Weight[is.na(tableau$Weight)]<-mean(tableau$Weight,na.rm=TRUE)
#tableau
#tableau$Quantity[is.na(tableau$Quantity)]mean(tableau$Quantity,na.rm=TRUE)
#tableau

#Joining the Data:
final<-inner_join(business1,tableau,.by.='GlobalTradeID')
